...
export default function AddStudentScreen() {
  ...
  const handleRegister = async () => {
    if (!name || !rollNumber || !imageUri) {
      Alert.alert('Error', 'Please fill in Name, Roll Number and provide a Photo');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Success', response.message || `${name} has been added to the class`, [
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Card style={styles.formCard}>
          <Text style={styles.subtitle}>Enter student information and provide a clear face photo for registration.</Text>
        ...
        <View style={styles.helpBox}>
          ...
          <Text style={styles.helpText}>
            A clear photo is required for the face recognition system to accurately mark attendance.
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  photoPlaceholder: { width: 120, height: 120, borderRadius: 60, backgroundColor: '#F1F5F9', borderStyle: 'dashed', borderWidth: 2, borderColor: '#CBD5E1', alignItems: 'center', justifyContent: 'center' },
  photoPickerText: { fontSize: 11, color: '#64748B', marginTop: 4, fontWeight: '500' },
  previewImage: { width: 120, height: 120, borderRadius: 60 },
  inputGroup: { marginBottom: 20 },
  label: { fontSize: 14, fontWeight: '600', color: '#475569', marginBottom: 8 },
  inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F1F5F9', borderRadius: 12, paddingHorizontal: 12 },
  inputIcon: { marginRight: 8 },
  input: { flex: 1, height: 48, fontSize: 16, color: '#0F172A' },
  submitBtn: { marginTop: 12 },
  helpBox: { flexDirection: 'row', gap: 12, backgroundColor: '#DBEAFE', padding: 16, borderRadius: 12, marginTop: 24 },
