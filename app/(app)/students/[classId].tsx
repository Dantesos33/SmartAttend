...
export default function StudentListScreen() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      if (email) {
        const [apiData, localClassStudents] = await Promise.all([
          getStudentsAPI(),
          storage.getObject<any[]>(`students_${email}_${classId}`),
        ]);

        const studentData = apiData.students || [];
        const localNames = (localClassStudents || []).map((s) => s.name);

        // Filter API students that are in this class locally
        const classFilteredStudents = studentData.filter((s: any) =>
          localNames.includes(s.name),
        );

        const mappedStudents = classFilteredStudents.map(
          (s: any, index: number) => ({
            id: index.toString(),
            name: s.name,
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              const email = await storage.getItem("userEmail");
              if (email) {
                // Also remove from local association
                const classIdKey = `students_${email}_${classId}`;
                const localClassStudents =
                  (await storage.getObject<any[]>(classIdKey)) || [];
                const updatedLocal = localClassStudents.filter(
                  (s) => s.name !== studentName,
                );
                await storage.setObject(classIdKey, updatedLocal);
          ...
          {loading && !refreshing ? (
            ...
            filteredStudents.map((student) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push({
                    pathname: "/(app)/student-stats/[studentId]",
                ...
                <Card style={styles.studentCard}>
                  ...
                  <View style={styles.studentInfo}>
                    ...
                    <Text style={styles.studentRoll}>
                      {student.roll || student.student_id || "No ID"}
                    </Text>
                  </View>
                  <View style={styles.attendanceInfo}>
                    ...
                    <Text style={styles.sessionsText}>
                      {student.present_count || 0}/{student.total_sessions || 0}
                    </Text>
                  </View>
                  <TouchableOpacity
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>
                {searchQuery ? "No results found" : "No students registered"}
              </Text>
            </View>
          )}
        </ScrollView>
